public static class ConstValue
{
    public const int SET_EFFECT_ENABLE_FURNITURE_COUNT = (int)FurnitureType.Length;
    public const int SET_EFFECT_ENABLE_KITCHEN_UTENSIL_COUNT = (int)KitchenUtensilType.Length;
    public const int ADD_ITEM_UPGRADE_COUNT = 2;
    public const int DEFAULT_MINIGAME_TIME = 30;
    public const int DEFAULT_EXCEPTIONAL_CUSTOMER_SPAWN_PERCENT = 10;

    //피버 관련 변수
    public const int MAX_PEVER_GAUGE = 100;
    public const float PEVER_TIME = 8f;


   //스테이지 관련 변수
    public const int MAX_SATISFACTION = 50;
    public const int MIN_SATISFACTION = -50; 
}
